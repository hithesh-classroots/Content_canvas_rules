...
2. **Roboto, pinned and self-hosted** — **bold** italic serif (Times New Roman) only on lowercase algebraic variables, each wrapped in a `<var>`/`.var` marker, and never on numerals, operators, function names, units, coefficients or vertex labels (§2.0).
...
116. **Keep the source layout unless it fails hierarchy, readability or information architecture** — and name the failure (§35).
