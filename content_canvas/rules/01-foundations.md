...
**Algebraic variables** — the one exception. Every variable glyph is **wrapped in a marker** — the HTML `<var>` element, or `class="var"` on an inline element (`<tspan class="var">` inside SVG, where `<var>` is not valid). The marker carries the face:
...
Bold italic serif for variables is the convention of every maths textbook and every typeset paper, and it earns its exception: it makes `a` the *unknown* visibly different from `a` the letter, and `x` visibly different from `×`. The weight is deliberate — a marked variable is bold **wherever it appears**: in an expression, on a tile, on a diagram label, and in the running prose about it. That weight is an exception to §2.2's weight-by-category (a variable stays 700 even inside a 500 sentence); it states *what kind of thing* the glyph is, an unknown, and the face already reads as set-apart rather than emphatic. Tinos and Liberation Serif are metric-compatible with Times New Roman and cover Linux and Android.

**The marker is not decoration — it is how the machine knows.** The face gate reads rendered styles and cannot tell an unknown `a` from a coefficient `a`; the `<var>`/`.var` marker is the one signal it trusts. So the bold italic serif is sanctioned on a marked glyph and on nothing else: serif italic on an **un**marked glyph reds as a stray vendor face, and marking a glyph is the author's explicit declaration that *this letter is the unknown*.
...
The marked bold italic serif applies to **lowercase single letters standing for an unknown quantity**: `x y z a b c n k r t`, and their subscripted forms (`x₁`, `aₙ`). Mark per glyph — wrap the letter alone, never the operators or numerals around it: `<var>x</var> + 3`, not `<var>x + 3</var>`. Whether a given `a` is a variable is the author's call made explicit by the marker: the same letter is the unknown in one problem and a fixed coefficient in the next, and only the mark says which.

It applies **everywhere that letter appears** — in the expression, in the prose about it, and in the label on a diagram. `aⁿ` in a formula and "the value of *a*" in the sentence below it are the same referent, so they wear the same face and carry the same marker (§34.6).
...
| Coefficients and parameters — the `a`, `b` in `ax + b` when they are fixed | A coefficient is a known value in this problem, not the unknown being solved for; leave it unmarked and upright |

**These stay Roboto upright by staying unmarked** — never wrap them in `<var>`/`.var`. The gate cannot tell a coefficient `a` from an unknown `a`, so the discipline is yours: mark only the unknown.

**A variable never carries an anchor colour and the bold italic at the same time** unless it is that board's focal point (§0.6). The face already distinguishes it; adding hue makes two signals for one fact.
...
- **Algebraic variables are the one exception to this table (§2.0).** A marked `<var>`/`.var` glyph is 700 bold italic serif *wherever it appears*, including inside a 500 sentence. There the weight is not emphasis — it is part of the face that marks the glyph as an unknown, and it does not count against the one-800-per-board rule.
...
| Correct | `#2f9e44` | `#40c057` | `#ebfbee` |
| Wrong | `#c92a2a` | `#fa5252` | `#fff5f5` |
...
**A component never decides whether it is light or dark — the room does** (`design-system.md §5`, source: Hithesh's *Dark Mode — Colour Adoption*). The canvas is transparent, so a board inherits the surface it sits on; that only works if every colour is a **role resolved per room**, never a fixed hex. The dark room is a **palette resolution, not a redesign** — every size, weight, radius, gap, dash and vertex is identical; only the palette moves. A component that hardcodes a light hex has silently chosen a room.

The roles above resolve to these dark values (measured ≥4.5:1 on `surface #262038`, ≥3:1 for large text and meaningful shapes):

| Role | Light | Dark | Paints |
|---|---|---|---|
| `canvas` | `#f8f5fe` grad | `#1a1526` | The room — **and the ink on any *light* accent fill** |
| `surface` | `#ffffff` | `#262038` | Cards, panels, bubbles, controls |
...
| `ink` | `#212529` | `#cdc7de` | Primary text |
| `dim` (`ink-body`) | `#495057` | `#918aac` | Secondary text, body copy |
| `faint` | `#adb5bd` | `#6b6485` | **Lines only — never text** |
| Concept | `#7950f2` | `#a385fa` | Violet — the concept, definitions |
| Emphasis | `#e8590c` | `#e8a53f` | Warm — the unknown, the pointed-at |
| Action | `#1c7ed6` | `#4da3f0` | Blue — interactive, live |
| Correct | `#2f9e44` | `#4cbf6e` | Success only |
| Wrong | `#c92a2a` | `#f07171` | Error only |
| `lattice` | `rgba(96,74,190,.13)` | `rgba(163,133,250,.10)` | The dot grid — **inverts: dark dots become light dots** |

**Two rules that are easy to get backwards** (`design-system.md §5`):

1. **A *light* accent fill takes `canvas` ink.** In the dark room a light accent (a `base` face, a correct/wrong fill) is bright, so white on it is only 1.4–2.4:1 — its ink is `canvas` (`#1a1526`). One role replaces the seven near-blacks a per-hue approach would need. **But** `base` is a *dark* fill in the *light* room, so the same rule inverted gives **white** there.
2. **A translucent scrim over media keeps `ink` (light) in *both* rooms** — the one place light ink is correct in the dark room (§3.5). Sweeping it to `canvas` gives near-black on near-black (~1.05:1). This is the exact inverse of rule 1, which is why a single-direction sweep cannot satisfy both.

Alpha tints of these roles are free, as are `rgba(0,0,0,…)`/`rgba(255,255,255,…)`. The full component-by-component treatment — buttons, sliders, counters, figures, the six inversions, adoption order and the audit — is `design-system.md §5`.
...
Correct → `#2f9e44`. Wrong → `#c92a2a`. A live value that changes as the student interacts → `#1c7ed6`. **Stop.**
...
**Reference implementation: `Design Component Systems.dc.html#shape`. It measures its own contrast at render time; trust it over any number written here.**
...
| Stop | Light `oklch()` | Dark `oklch()` | Painted on |
|---|---|---|---|
| `fill` | `92% .12 h` | `26% .05 h` | Surface at rest — the term's own chip |
| `fill2` | `84% .155 h` | `32% .07 h` | Nested surface, chart band, stronger tint |
| `base` | `48% .19 h` | `62% .15 h` | Highlight stroke, button face, plotted marker |
| `edge` | `36% .165 h` | `50% .13 h` | A button's base edge — the face's own hue, darker |
| `text` | `40% .155 h` | `88% .06 h` | Label on that fill |

**The ladder inverts, and it is the step a hex sweep cannot do** (`design-system.md §5`). Read the two columns against each other: **surface stops (`fill`, `fill2`) cross from near-white to near-black**; **`text` goes light** because it is the ink for those surfaces; and **`base` crosses the middle** — a *dark* fill in the light room, a *light* fill in the dark room — which is exactly why ink on a `base` face is **white** in the light room and **`canvas`** in the dark one (§3.1a). A `fill` authored at `92%` stays a near-white plate wherever it lands and will *pass* a contrast audit, because it pairs a light fill with dark ink and is internally consistent as a light-mode component. The rule that closes it: **a tint is an alpha of its accent, or a per-room lightness — never a fixed value, in any colour space.**
...
**In the dark room, shadows are pure black — depth, never brand** (`design-system.md §5`). A hued glow on near-black reads as a smear, and large soft shadows smudge, so keep them tight and raise the alpha instead of the blur:

```
resting  0 2px 7px rgba(0,0,0,.20)
raised   0 18px 44px rgba(0,0,0,.40)
```

The one white allowed in a dark-room shadow is the lit top edge that gives a filled control its form — `inset 0 1px 0 rgba(255,255,255,.32)` — which replaces the light room's button gradient (a two-stop gradient inside one token is dead weight here).
