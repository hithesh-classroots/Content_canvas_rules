...
- A face other than Roboto visible beside canvas text (§2.0) — except a `<var>`/`.var`-marked algebraic variable, which is bold italic serif by rule
...
Also: strip every `font-family` declaration (the stack is inherited), every `italic` (not in the system), every `text-transform:uppercase` that isn't on a `label`, and every `letter-spacing` outside the four steps' values. **The one exception is a `<var>`/`.var` marker** (§2.0): keep its serif family, its italic and its 700 — that face *is* the system for a variable, and stripping it turns an unknown back into an ordinary letter.
...
5  TYPE        reclassify weight by category; snap sizes; strip families, italics, tracking — but keep a `<var>`/`.var` marker's serif italic 700 (§2.0)
