...
| A serif variable that isn't marked | Bold italic serif is sanctioned *only* on a `<var>`/`.var` glyph (§2.0); unmarked, the gate can't tell it from a stray face and reds it |

**Algebraic variables are the one carve-out.** A glyph that is the unknown wears bold italic serif — but it must be marked, or the face gate reds it as a vendor face. Mark the letter alone:

```html
The area is <var>b</var> × <var>h</var>.        <!-- HTML -->
<tspan class="var">x</tspan>                     <!-- inside SVG -->
```

```css
var, .var { font-family: "Times New Roman", Tinos, "Liberation Serif", Georgia, serif; font-style: italic; font-weight: 700; }
```

Mark only the unknown — never a coefficient, a numeral, an operator or a unit (§2.0a).
