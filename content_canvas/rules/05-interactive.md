...
**Reference implementation: `Design Component Systems.dc.html#slider`. It measures its own contrast at render time across every hue; trust it over any number written here.**
...
**Reference implementation: `Design Component Systems.dc.html#counter`. It measures its own contrast at render time across every hue; trust it over any number written here.**
...
| Wrong | 2px `#c92a2a`, fill `#fff5f5`, `ti-x` outside, right. **The student's value stays** — clearing it removes the thing they need to look at |
...
**Reference implementation: `Design Component Systems.dc.html#shape`, which measures its own contrast at render time.** This section supersedes the earlier per-component geometry: everything that holds content or is pressed is one of these two families, and nothing else exists.
...
| Button / Text State | Style Description |
|---|---|
...
> **Two headers restored 2026-08-19, by inference (Aswin) - not recovered.** The rows above
> were orphaned: they carried pipes with no header, so they rendered as literal text to every
> seat, and the loss predates this repo - the same two fragments sit at the same place in the
> monolith. `Button / Text State | Style Description` and `Kind | Style | Use` fit the surviving
> cells and the file's own idiom; **no cell text was altered, added or removed.** Awaiting
> Hithesh's confirmation - `.github/open-questions.txt`.
...
#### 20.0.5 The dark room — buttons, sliders, counters

The dark room is a palette resolution (§3.1a), but these three controls each carry a **specific inversion that a light→dark sweep gets wrong** — and every one has shipped as a real defect (`design-system.md §5`, source: Hithesh's *Dark Mode — Colour Adoption*). This is the section for a seat building a control that "looks off" in dark.

**Buttons — the six states resolve like this in the dark room:**

| State | Dark treatment | The trap |
|---|---|---|
| Primary / Highlighted | `base` face, **`canvas` ink** — never white | `base` is a *light* stop in dark, so white on it is 1.4–2.4:1 |
| Default (neutral) | `36% .085` — hue must come from **chroma**, not lightness | at `.022` (the light value) it is flat grey and reads as disabled |
| Selected | **`fill2`** face, `text` ink | `fill` alone is level with `canvas` (L .19) and reads as a hole; a face must sit **above** its room — 32–36% is the floor |
| Correct / wrong | `correct` / `wrong` face, **`canvas` ink** | white on the light state fill fails |
| Disabled | fill **below** the live face, ink `dim` | recede on **lightness**, not only hue — a disabled fill *lighter* than the live face still reads as raised |

**Sliders — the bar leads and the thumb is the brightest thing:**

- **Track, unfilled** — a dark surface (`30% .05`), never a pale tint.
- **Track, filled** — `base`, which is a *light* stop in dark, so the filled portion is the **bright** part. This is the whole inversion: in light the fill darkens the bar, in dark it lightens it.
- **Thumb** — the one element a student touches, so it is the **brightest thing on the control**: `ink` with a lighter rim. Do **not** tie it to a resting-surface value; in the light room that value was near-white, which is what made it read as the handle — in dark that same value is near-black and the thumb vanishes.
- **Thumb drop face** (`--edge`) — stays **darker** than the thumb; it is the ledge the handle casts, not a rim highlight.
- **Notches** — off `46% .10`, on `66% .14`. **Counter chip** — a light `base`-family face with `canvas` ink.

**Counters:**

- **Value tile** — dark fill, **light `text` ink**. The single most common defect: the fill is inverted while the ink stays `deep`, so both end up dark and the numeral is *invisible*, not merely dim.
- **± buttons** — `base`-family face, `canvas` ink. **Disabled** — fill just above `surface`, ink `faint`; recessive on both axes.

Disabled recessive is **not** illegible: a disabled numeral still owes 4.5:1 against its own fill (§3.1a, the six inversions).
...
- **Correct:** row border → 2px `#40c057`, fill `#ebfbee`, text `#2f9e44`, `ti-check` badge, ring pulse once.
- **Wrong:** row border → 2px `#fa5252`, fill `#fff5f5`, text `#c92a2a`, `ti-x` badge. **No shake, no buzz.**
